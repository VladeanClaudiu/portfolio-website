import Footer from "./Footer"
import Header from "./Header"
export default function About() {
    return (
        <div className="main-section">
            <Header 
            />
            <main>
               <h1>Hello World, I'm Claudiu</h1>
                <p>
                    I'm a Frontend / React Developer, 
                    I am passionate about creating interactive web apps alongside 
                    problem-solving and learning new technologies by testing myself 
                    and developing personal projects in my free time. 
                </p>
                <p>
                    I aim to make my code as reusable as possible and  I enjoy working with CSS due to my love for design and typography.
                </p> 
                <h3>I'm currently...</h3>
                <ul>
                    <li>working on improving my React.js skills, as I search for a new position in Frontend development.</li>
                    <li>working out as much as I can in my free time.</li>
                    <li>looking into the riot games api and work on a personal project dashboard, 
                        Where all my character stats from every Riot game can be viewed.</li>
                </ul>
            </main>
            
            
            <Footer />
        </div>
    )
}